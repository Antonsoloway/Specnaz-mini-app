...
- GitHub source переключён на v0.5.59 18.08.2026; фактический Telegram smoke-test после последних UI-фиксов нужно считать отдельным production-подтверждением.
...
- `v0.5.59` — статус активных команд спецназа, золотая маркировка, центрированный значок, каталог активных команд с независимыми фильтрами/поиском; в credits добавлен `@DmitryRoyal`.
...
11. Бейдж версии = `v0.5.59`, история открывает v0.5.59 первой.
12. На странице `Команды` активные команды в золотой рамке, paused — без неё.
13. На странице `Участники` плашки активных команд в золотой рамке.
14. `⚡️ MOLOT POKA / РК`: золотое название в рамке + специальный значок справа.
15. Значок активной команды полностью виден и центрирован, без серой заглушки/обрезки.
16. Нажатие на значок открывает `Команды принимающие участие в спецназе` и показывает только `status = Активен`.
17. В каталоге работают `Все / РМ / РК`, счётчик меняется корректно.
18. Поиск каталога ищет по названию/игре и работает совместно с его фильтром, не меняя обычную страницу `Команды`.
19. В credits виден `@DmitryRoyal`.
