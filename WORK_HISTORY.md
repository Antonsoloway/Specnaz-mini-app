...
## 2026-08-19 — v0.5.59: ускорено открытие кэшированных фото команд на iPhone
...
- это отдельный performance-дефект после предыдущего исправления «фото мелькнуло → исчезло».

**Точная причина:**
- общий `media-persistent-cache-v0554.js` умеет поднимать team blobs из IndexedDB в `teamMemory`, но старый `warmTeamCacheFromDisk()` делает `idbGet()` **последовательно для каждой команды**;
- для каждого найденного фото дополнительно запускается `idbTouch()` с отдельной write transaction;
- на iOS IndexedDB эта цепочка значительно медленнее, поэтому пользователь успевает открыть команду раньше, чем нужный blob попал в session memory;
- Android выполняет тот же путь заметно быстрее, поэтому задержка почти не видна.

**Исправлено:**
- `stable-v0559.js` поднят до stable patch **`0.5.59.3`**;
- добавлен iOS-only fast team-photo memory layer;
- сразу при загрузке JS запускается **одно пакетное readonly-чтение** media store (`getAll`, cursor fallback), ещё до авторизации/переходов;
- сохранённые `kind=team` blobs младше 45 дней превращаются в session object URLs без сетевого prewarm и без массового `idbTouch`;
- `pointerdown` по любому `[data-team]` дополнительно делает точечный `get` конкретной команды, если общий warm ещё не завершился;
- после `renderTeamDetail()` найденный cached object URL назначается синхронно, с `loading=eager`, `decoding=sync`, `fetchPriority=high`;
- если iOS fast source уже назначен, старый async team-photo loader не запускает повторно медленный путь;
- при ошибке fast source marker снимается и включается штатный loader/fallback;
- предыдущий iOS guard, запрещающий удалять рабочий `src` до готовности replacement, сохранён;
- Android ветка не менялась;
- `app-v0559.html` cache-bust: `stable-v0559.js?v=20260819-1845`, `changelog-v0559.js?v=20260819-1845`.

**Важно:** fast warm читает только уже сохранённые IndexedDB blobs. Он **не скачивает все фото по сети на старте**.

**Инвариант:** на iPhone уже кэшированные team photos должны быть подняты в session memory заранее одним пакетным чтением, а не ждать последовательного per-team IndexedDB warm после открытия раздела.
