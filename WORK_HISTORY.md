...
**Проверка данных до повтора:** private snapshot `2026-08-21T18:03:56.360Z` показывает участника `1456874273 / Антон` без `CATAHA` — только прежние `Спецназ РМ` и `Спецназ РК`; у команды row 99 по-прежнему players=0; в journal остаётся только прежний `createTeam`. Значит отказ безопасный, частичной записи и дубля нет.

**Причина:** пятиминутный `MINIAPP_exportUnifiedSnapshotToGitHub()` держит общий Apps Script `ScriptLock` во время чтения таблиц и GitHub export. Admin backend корректно ждёт lock до 20 секунд и возвращает explicit `WRITE_BUSY`, если фон ещё не закончен. Повтор этой конкретной ошибки безопасен, потому что сервер гарантирует, что mutation не стартовала.

**Исправление build `20260821-2110`:** `admin-write-v0600-v3.js` → `.4`. Client автоматически повторяет только `WRITE_BUSY` максимум два раза с задержками 1.5/3 секунды и тем же `requestId`; server idempotency сохраняется. Отдельно остаётся максимум один retry настоящего transport failure с тем же id. Validation, stale revision и destructive conflicts автоматически не повторяются. UI показывает процесс ожидания lock вместо немедленного красного отказа.

**Проверка:** 20/20 Node tests, JS syntax checks и `git diff --check` прошли. Добавлен regression assertion для bounded `WRITE_BUSY` retry/same-id; Apps Script/Sheets во время исправления не менялись. После публикации нужен один Android smoke: заново открыть build 2110 и один раз добавить Антона в `CATAHA`.
