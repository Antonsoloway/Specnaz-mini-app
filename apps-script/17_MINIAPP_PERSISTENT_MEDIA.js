...
var MINIAPP_PERSISTENT_MEDIA_VERSION = '1.0.0';
...
function MINIAPP_syncOneTeamPhotoRow_(sheet, row, cols, cfg, force) {
  ...
  var stableHash = MINIAPP_mediaSha256Hex_(MINIAPP_normalizeTeamMediaName_(teamName));
  ...
  // Critical: getContentUrl() is short-lived, so fetch it immediately in the
  // same Apps Script execution, then persist only the binary bytes.
  var response = UrlFetchApp.fetch(contentUrl, {
    method: 'get',
    muteHttpExceptions: true,
    followRedirects: true,
    headers: { 'User-Agent': 'Royal-CRM-Team-Media/1.0' }
  });
...
function MINIAPP_teamHeaderColumns_(sheet) {
  ...
  // Current CRM layout fallback: B=Команда, C=Фото.
  if (!team) team = 2;
  if (!photo) photo = 3;
