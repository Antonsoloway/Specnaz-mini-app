 ...
 * Uses a dedicated stable HTML entry so Telegram cannot reuse an older WebView page.
 */

var MINIAPP_BOT_APP_MENU_VERSION = '1.0.10';
var MINIAPP_BOT_APP_URL = 'https://antonsoloway.github.io/Specnaz-mini-app/app-v0513.html';
...
function MINIAPP_setupBotAppMenu() {
  ...
  var appUrl = MINIAPP_BOT_APP_URL + '?cb=20260815-2053';
  if (gasUrl) appUrl += '&gas=' + encodeURIComponent(gasUrl);

  var api = 'https://api.telegram.org/bot' + token + '/';

  var commands = MINIAPP_botAppMenuCall_(api + 'setMyCommands', {
    commands: [
  ...
  return {
    ok: true,
    version: MINIAPP_BOT_APP_MENU_VERSION,
    appUrl: appUrl,
    menuButton: verify && verify.ok ? verify.result : null
  };
}

function MINIAPP_botAppMenuCall_(url, payload) {
  ...
  body.httpCode = response.getResponseCode();
  return body;
}
