 ...
 * Search keys are prepared here, not on the phone.
 */

var MINIAPP_UNIFIED_SNAPSHOT_VERSION = '1.1.0';
var MINIAPP_UNIFIED_SNAPSHOT_SCHEMA = '1.4.0';
var MINIAPP_UNIFIED_SEARCH_INDEX_VERSION = '1.0.0';
...
function MINIAPP_exportUnifiedSnapshotToGitHub() {
  ...
  try {
    ...
    // Prepare all search forms once on the server. Mobile clients only compare strings.
...
function MINIAPP_unifiedBuildSearchKeys_(values) {
  ...
  function visit(value) {
    ...
    if (/[a-z]/i.test(n) && !/[а-я]/u.test(n)) add(MINIAPP_unifiedLatToCyr_(n));
...
function MINIAPP_unifiedCyrToLat_(value) {
  ...
  return Array.from(MINIAPP_unifiedNormalizeSearch_(value)).map(function(ch) { return Object.prototype.hasOwnProperty.call(map, ch) ? map[ch] : ch; }).join('');
}
