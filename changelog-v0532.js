/* Royal CRM Mini App — changelog v0.5.32 */
(() => {
  ...
  const RELEASES = [
    {
      ...
      changes: [
        ...
        'Исправлена потеря Telegram-авторизации при переходе со старых адресов Mini App: launch hash с tgWebAppData теперь сохраняется целиком.'
      ]
    },
    {
      version: '0.5.31',
    ...
    ...previous.filter(item => !['0.5.32', '0.5.31', '0.5.30'].includes(String(item?.version || '')))
  ];
