/* Royal CRM Mini App — changelog v0.5.53 */
(() => {
  ...
  const RELEASES = [{
    version:'0.5.53',
    title:'Поисковые ключи дошли до приложения',
    changes:[
      'Исправлен Worker: готовые searchKeys из snapshot теперь передаются в Mini App вместо удаления при выдаче данных.',
      'Добавлено одно лёгкое детерминированное чтение псевдорусских названий без перебора вариантов: CKAZKA → сказка, BEHOM → веном.',
      'Сопоставление карточки команды с данными snapshot усилено: используется и data-team, и видимое название карточки.',
      'Кэш поиска сбрасывается при смене загруженного snapshot, поэтому новые ключи начинают работать без устаревшего haystack.'
    ]
  }, ...previous];
