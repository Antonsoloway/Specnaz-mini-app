/* Royal CRM Mini App — changelog v0.5.54 */
(() => {
  ...
  const RELEASES = [{
    version:'0.5.54',
    title:'Постоянный кэш аватаров и фото',
    changes:[
      'Аватары участников и фотографии команд после первой загрузки сохраняются в IndexedDB на устройстве и при следующих открытиях берутся из локального кэша.',
      'Аватары кэшируются по avatarFileId, поэтому смена аватара автоматически создаёт новую запись; фото команд привязаны к актуальному photoUrl.',
      'На слабом интернете одновременно загружается не более двух аватаров, а очередь формируется только для изображений рядом с видимой областью экрана.',
      'Неиспользуемые изображения автоматически очищаются: срок хранения до 45 дней, общий лимит кэша — 420 записей.'
    ]
  }, ...previous];
