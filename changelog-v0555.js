/* Royal CRM Mini App — changelog v0.5.55 */
(() => {
  ...
  const RELEASES = [{
    version:'0.5.55',
    title:'Мгновенная своя аватарка',
    changes:[
      'Собственная аватарка на главной странице теперь загружается приоритетно сразу после авторизации, без ожидания общего списка и IntersectionObserver.',
      'Для своей аватарки создаётся дополнительная постоянная запись кэша по Telegram ID, поэтому её можно восстановить из IndexedDB ещё до загрузки snapshot.',
      'Карточка профиля больше не ждёт avatarFileId, чтобы создать элемент изображения: локальная аватарка может появиться сразу вместо временной буквы.'
    ]
  }, ...previous];
