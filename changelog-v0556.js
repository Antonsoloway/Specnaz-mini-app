/* Royal CRM Mini App — changelog v0.5.56 */
(() => {
  ...
  const RELEASES = [{
    version:'0.5.56',
    title:'Аватар без мигания',
    changes:[
      'Убрано краткое появление буквы-заглушки при повторной отрисовке своей карточки после загрузки snapshot.',
      'Уже показанная собственная аватарка сохраняется в памяти текущего запуска и подставляется синхронно в новую карточку до следующего кадра.',
      'Восстановление своей аватарки больше не откладывается через setTimeout после перерисовки профиля.'
    ]
  }, ...previous];
