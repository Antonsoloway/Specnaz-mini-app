...
(() => {
  ...
  const RELEASES = [{
    ...
    changes:[
      ...
      'Фото команд теперь кэшируются по стабильной связке «команда + игра», а не по временным Google URL.',
      'Кэшированные фото команд ускорены безопасно: IndexedDB заранее читается одним readonly-проходом в родную память медиакэша, а blob URL создаётся только для реально открываемой команды; сетевой prewarm не используется.',
