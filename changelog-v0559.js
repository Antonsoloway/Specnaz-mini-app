...
(() => {
  ...
  const RELEASES = [{
    ...
    changes:[
      ...
      'У участников без @username отображается кнопка «Связаться»: Голубец отправляет в ЛС настоящую Telegram inline-кнопку «Открыть профиль» по raw Telegram ID, после чего приложение открывает чат с Голубем. Прямой tg://user?id из Mini App не используется.',
