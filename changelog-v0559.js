...
(() => {
  ...
  const RELEASES = [{
    ...
    changes:[
      ...
      'В карточке активной команды название выделено золотым цветом и золотой рамкой, справа добавлен специальный значок.',
      'Значок активной команды исправлен: изображение центрируется как фон кнопки и больше не попадает под обработку обычных изображений.',
      'На странице активных команд добавлены отдельные фильтры «Все / РМ / РК» и строка поиска по названию команды или игре.',
      'В поиск добавлен подтверждённый алиас: команда BbIIIIKA в Royal Kingdom находится по запросу «вышка».',
      'Нажатие на значок открывает отдельную страницу «Команды принимающие участие в спецназе» со всеми активными командами.'
    ]
  }, ...previous];
