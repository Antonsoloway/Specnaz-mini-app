...
(() => {
  ...
  const RELEASES = [{
    ...
    changes:[
      ...
      'На iPhone ускорено повторное открытие фото команд: сохранённые team blobs одним пакетным чтением IndexedDB поднимаются в память ещё при старте, а нужная команда дополнительно прогревается уже на касании до открытия карточки.',
