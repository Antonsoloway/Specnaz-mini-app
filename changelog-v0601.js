  ...
  const SECTIONS = [
    {
      ...
      changes:[
        'Исправлено смешивание фотографий у разных команд с одинаковым текстовым названием: значок в названии и игра теперь входят в точную identity команды, поэтому, например, команды с разными эмодзи больше не получают общее фото.',
        'Неактивные команды больше не показываются в публичной части приложения. Они остаются доступны в админ-режиме как неактивные записи.',
        'Исправлено редактирование участника в Telegram на iPhone: поля имени, игры, роли, команды и игрового ника снова работают нативно и не блокируются touch-обработчиками админской формы.',
        'Исправлен чёрный экран при нажатии на свою команду на главной: карточка команды теперь открывается в видимом основном panel и Back возвращает в собственный профиль.',
        'В админ-редактор участника добавлен раздел «Награды». Награды хранятся в расширяемом каталоге; первой управляемой ачивкой стал «МАЯК».',
        'Ачивка «МАЯК» стала единым источником принадлежности к проекту: выдача автоматически добавляет участника в список проекта и показывает badge, снятие убирает оба проявления.',
        'Добавление и снятие ачивок администраторами теперь записывается в общий Журнал: видно администратора, участника и конкретный diff награды; сохранение без фактического изменения не создаёт дубликат события.',
        'Добавлен живой осенний фон с падающими кленовыми листьями за интерфейсом. Эффект не перехватывает касания и автоматически останавливается, когда Mini App скрыт.',
        'После проверки на реальном Telegram WebView исправлена невидимость осенних листьев: убрано полное отключение при reduced-motion, используется более совместимый Canvas-контекст, слой поднят над фоном, а сами листья сделаны немного заметнее при сохранении ненавязчивой плотности.',
        'Осенние листья сделаны ярче и разнообразнее: добавлена расширенная жёлто-оранжево-красно-бордово-коричневая палитра, размеры теперь заметно различаются от маленьких до крупных, а прозрачность самих листьев убрана.',
        'История изменений v0.6.1 синхронизирована с последними релизами и получила тот же cache-bust, что и активная сборка, чтобы Telegram не показывал старую карточку истории после обновления.'
      ]
    },
    {
      ...
      changes:[
        ...
        'Свежий production snapshot уже содержит восстановленные avatarFileId для ранее проблемных карточек.',
        ...
        'Последующие исправления, которые входят в v0.6.1, дописываются в эту карточку истории изменений вместе с релизом.'
      ]
    }
  ];
