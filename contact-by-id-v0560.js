/* Royal CRM Mini App — contact by Telegram ID v0.5.60
...
(() => {
  ...
  function contactButtonHtml(id, compact = false) {
    ...
    if (!clean) return '';
    return `<button type="button" class="username-link contact-by-id-v0560${compact ? ' compact' : ''}" data-contact-telegram-id="${clean}" aria-label="Связаться с участником">Связаться</button>`;
  }
  ...
  function makeButton(id, compact = false) {
    ...
    button.type = 'button';
    button.className = `username-link contact-by-id-v0560${compact ? ' compact' : ''}`;
  ...
  function decoratePersonCard(card) {
    ...
    if (!main) return;
    const button = makeButton(id, false);
    insertBeforeFirst(main, button, ['.telegram-name', '.membership-list']);
  }
  ...
  function decorateTeamMember(card) {
    ...
    if (!main) return;
    const button = makeButton(id, true);
    insertBeforeFirst(main, button, ['.telegram-name', '.team-member-role', '.membership-list']);
  }
  ...
  function decorateDirectoryCard(card) {
    ...
    const button = makeButton(id, true);
    insertBeforeFirst(main, button, ['.telegram-name', '.directory-person-meta', '.membership-list']);
  }
  ...
  function decorateHeroCard(card) {
    ...
    if (!main) return;
    const button = makeButton(id, true);
    insertBeforeFirst(main, button, ['.hero-meta', '.membership-list']);
  }
  ...
  function openTelegramProfileById(value) {
    ...
    // Telegram WebView reliably hands tg:// links back to the Telegram client
    // when they are activated by a real user gesture. Do not route this through
    // WebApp.openTelegramLink(), which is intended for https://t.me links.
    try {
