/* Royal CRM Mini App — participant profiles v0.5.15
 * Identity-safe: participant history is matched ONLY by participantKey.
 * participantKey is derived server-side from Telegram ID; names/usernames are never used for history ownership.
 */
(() => {
  ...
  function resolveParticipant(avatar) {
    ...
    const directKey = String(avatar.dataset?.participantKey || '').trim();
    if (directKey) return findByKey(directKey);

    if (avatar.classList.contains('self-avatar')) {
      const selfKey = String(authState?.user?.participantKey || '').trim();
      if (selfKey) return findByKey(selfKey);
    }
    return null;
  }
  ...
  function participantHistory(p) {
    ...
    sections.forEach(section => {
      ...
      rows.forEach(row => {
        if (String(row?.participantKey || '').trim() === key) {
  ...
  function renderParticipantProfile(p) {
    ...
    const avatar = avatarFile
      ? `<div class="participant-detail-avatar"${keyAttr}><span>${esc(firstLetter(name))}</span><img alt="" data-avatar-file="${esc(avatarFile)}"></div>`
      : `<div class="participant-detail-avatar fallback"${keyAttr}><span>${esc(firstLetter(name))}</span></div>`;
  ...
  window.__ROYAL_PARTICIPANT_PROFILE_VERSION__ = '0.5.15';
})();