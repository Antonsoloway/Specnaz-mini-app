/* Royal CRM Mini App — participant profiles v0.5.18
 * Participant identity: raw Telegram ID only.
 * No name/username/team fallback is used for ownership or history matching.
 */
(() => {
  ...
  function restoreOrigin() {
    ...
    const panel = document.getElementById('panel');
    if (panel && saved.panelHtml) {
      setActiveNav(saved.page || 'players');
      panel.hidden = false;
      panel.innerHTML = saved.panelHtml;
      try { setupAvatarLoading(panel); } catch (_) {}
    } else {
      renderPage(saved.page || 'players');
    }
