...
(() => {
  ...
  function rankPremium(rank, score) {
    try { return window.RoyalRank?.premium?.(rank, score) || `<div class="participant-detail-rank-chip">${esc(rank || 'Новичок')}</div>`; }
    catch (_) { return `<div class="participant-detail-rank-chip">${esc(rank || 'Новичок')}</div>`; }
  }
  ...
  function historyCardHtml(row) {
    ...
    return `<article class="participant-trip-card">
      ...
      <div class="participant-trip-score"><span>Было <b>${esc(row?.before || '')}</b></span><span>→</span><span>Стало <b>${esc(row?.after || '')}</b></span>${row?.rank ? `<span class="participant-trip-rank">${rankCompact(row.rank, after, true)}</span>` : ''}</div>
  ...
  function renderByTelegramId(value) {
    ...
    panel.innerHTML = `
      ...
      <section class="participant-detail-card">
        <div class="participant-detail-head">${avatar}<div class="participant-detail-identity"><div class="participant-rank-inline">${rankCompact(rank, trips, true)}</div><h2>${esc(name)}</h2>${username ? `<button type="button" class="username-link" data-user-menu="${esc(username)}" data-user-name="${esc(name)}">@${esc(username)}</button>` : ''}</div></div>
        ${rankPremium(rank, trips)}
