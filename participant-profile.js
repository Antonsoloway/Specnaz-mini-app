/* Royal CRM Mini App — universal participant profile v0.5.11 */
(() => {
  const UI_VERSION = '0.5.11';
  ...
  function findByName(value, contextText = '') {
    ...
    if (!wanted) return null;
    let matches = participants().filter(p => participantNames(p).includes(wanted));
    if (matches.length === 1) return matches[0];
    if (matches.length > 1 && contextText) {
      const hay = norm(contextText);
      const narrowed = matches.filter(p => (p?.memberships || []).some(m => hay.includes(norm(m?.team)) && norm(m?.team)));
  ...
  function currentSelfParticipant() {
    ...
    const byUser = findByUsername(user.crmUsername || user.telegramUsername || '');
    if (byUser) return byUser;
    const avatar = String(authState?.profileStats?.avatarFileId || '').trim();
    const byAvatar = findByAvatarFile(avatar);
    if (byAvatar) return byAvatar;
    return findByName(user.crmName || user.telegramFirstName || '');
  }
  ...
  function participantHistory(p) {
    ...
    sections.forEach(section => {
      (Array.isArray(section?.rows) ? section.rows : []).forEach(row => {
        if (rowMatchesParticipant(row, p)) result.push({ ...row, sectionTitle: String(section?.title || '') });
      });
    });
  ...
  function captureOrigin() {
    ...
    if (page === 'teams') {
      ...
      if (title) data.teamRef = JSON.stringify([title.trim(), game.trim()]);
      else data.query = document.getElementById('teamSearch')?.value || '';
    }
  ...
  function restoreOrigin() {
    const saved = origin;
    origin = null;
    if (!saved) { renderPage('players'); return; }

    if (saved.page === 'players') renderParticipantsPage(saved.query || '');
    else if (saved.page === 'teams' && saved.teamRef) renderTeamDetail(saved.teamRef);
    ...
    try { window.scrollTo({ top: saved.scrollY || 0, behavior: 'instant' }); } catch (_) { window.scrollTo(0, saved.scrollY || 0); }
  }
  ...
  function markAvatars(root = document) {
    root.querySelectorAll?.(AVATAR_SELECTOR).forEach(node => {
      node.classList.add('participant-avatar-clickable');
      node.setAttribute('role', 'button');
      node.setAttribute('tabindex', '0');
      node.setAttribute('aria-label', 'Открыть профиль участника');
    });
  }
  ...
  document.addEventListener('click', event => {
    ...
    const p = resolveParticipant(avatar);
    if (!p) return;
    event.preventDefault();
    event.stopPropagation();
  ...
  const observer = new MutationObserver(mutations => {
    mutations.forEach(m => m.addedNodes.forEach(node => {
      if (node.nodeType === 1) {
        if (node.matches?.(AVATAR_SELECTOR)) markAvatars(node.parentElement || node);
        else markAvatars(node);
      }
    }));
  });
  ...
  window.__ROYAL_PARTICIPANT_PROFILE_VERSION__ = UI_VERSION;
})();
