/* Royal CRM Mini App — self profile card v0.5.23, Telegram ID only */
(() => {
  ...
  function rankCompact(rank, score) {
    try { return window.RoyalRank?.compact?.(rank, score, { tiny:false, label:true }) || esc(rank || 'Новичок'); }
  ...
  function profileInnerHtml() {
    ...
    return `<div class="self-profile-head">${avatar}<div class="self-profile-identity"><span class="self-role-chip">${esc(role)}</span><h2>${esc(name)}</h2>${username ? `<div class="self-username">@${esc(username)}</div>` : ''}</div></div><div class="self-profile-stats"><div class="self-stat"><b>${trips}</b><small>Походы в спецназ</small></div><div class="self-stat rank"><div class="self-rank-badge">${rankCompact(rank, trips)}</div><small>Звание</small></div></div><div class="self-memberships">${memberships.length ? memberships.map(membershipLine).join('') : '<span class="muted">Командные роли не указаны</span>'}</div><div class="self-crm-status muted">${esc(crmStatus)}</div>`;
  }
