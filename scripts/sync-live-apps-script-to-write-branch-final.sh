...
stage "FINAL WRITE-BRANCH MIRROR PREFLIGHT"
...
[[ "$COUNT" -ge 20 ]] || fail "Подозрительно мало live-файлов: $COUNT"
for required in \
  12_MINI_APP_API \
  28_MINIAPP_ADMIN_DATA \
  29_MINIAPP_ADMIN_WRITE \
  30_MINIAPP_ADMIN_WRITE_BACKEND \
  31_MINIAPP_ADMIN_WRITE_HARDENED \
  32_MINIAPP_ADMIN_TEAM_PHOTO \
  33_MINIAPP_ADMIN_WRITE_FINAL; do
...
{
  echo '# Live Apps Script mirror — v0.6 final write branch'
  echo
...
git commit -m "Sync v0.6 final live Apps Script ${STAMP}" >/dev/null \
