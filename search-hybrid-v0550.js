 ...
 * Adds game scope (Все / РМ / РК) and keyboard dismissal on scroll/tap outside.
 */
(() => {
  ...
  function gameCodesFromTeam(team) {
    const out = new Set();
    const values = [team?.game, ...(Array.isArray(team?.games) ? team.games : [])];
  ...
  function teamForCard(card) {
    ...
    if (cardCodes.size) {
      const exact = candidates.find(team => {
        const codes = gameCodesFromTeam(team);
        return [...cardCodes].some(code => codes.has(code));
      });
  ...
  function itemMatchesGame(item, card, kind, filterCode) {
    ...
    const codes = gameCodesFromTeam(item);
    if (codes.has(filterCode)) return true;
    return gameCodesFromCard(card).has(filterCode);
  }
  ...
  function allCombinedHaystack(card, kind) {
    const cached = allCardCache.get(card);
    ...
    allCardCache.set(card, haystack);
    return haystack;
  }

  function scopedCombinedHaystack(card, kind, filterCode) {
    if (filterCode === GAME_ALL) return allCombinedHaystack(card, kind);

    if (kind === 'teams') {
      // A team card already belongs to the selected game, so all of its old local
      // search + searchKeys remain valid and additive.
      return allCombinedHaystack(card, kind);
    }

    let scoped = scopedCardCache.get(card);
    if (!scoped) {
      scoped = {};
      scopedCardCache.set(card, scoped);
    }
    if (scoped[filterCode]) return scoped[filterCode];

    const item = snapshotItemForCard(card, kind);
    // Under a game filter, participant membership text is scoped to that game.
    // This prevents an RK team/nickname from matching while the РМ filter is active,
    // while person name / @username remain searchable in both games they belong to.
    const haystack = makeLocalHaystack(participantScopedText(item, filterCode));
    scoped[filterCode] = haystack;
    return haystack;
  }
  ...
  function ensureEmpty(container) {
    ...
    if (!empty) {
      ...
      empty.className = 'empty-state search-empty-v0550';
  ...
  function updateCount(title, found, total, eligible, hasQuery, filterCode) {
    ...
    if (filterCode === GAME_ALL) {
      muted.textContent = hasQuery ? `${found} из ${total}` : String(total);
      return;
    }
    muted.textContent = hasQuery ? `${found} из ${eligible}` : `${eligible} из ${total}`;
  }
  ...
  function filterRendered(kind, rawQuery) {
    ...
    const filterCode = filters[kind] || GAME_ALL;
  ...
  function schedule(kind, value) {
    ...
    if (kind === 'participants') {
      ...
      participantTimer = setTimeout(() => filterRendered(kind, queries[kind]), FILTER_DELAY);
    } else {
      clearTimeout(teamTimer);
      teamTimer = setTimeout(() => filterRendered(kind, queries[kind]), FILTER_DELAY);
    }
  }
  ...
  function updateFilterButtons(kind) {
    document.querySelectorAll(`.royal-game-filter-btn[data-search-kind="${kind}"]`).forEach(btn => {
      const active = btn.dataset.gameFilter === filters[kind];
  ...
  function installFilter(kind) {
    const inputId = kind === 'participants' ? 'participantSearch' : 'teamSearch';
    const input = document.getElementById(inputId);
    const box = input?.closest?.('.search-box');
    if (!box) return;

    let bar = box.previousElementSibling;
    if (!bar?.classList?.contains('royal-game-filter')) {
      box.insertAdjacentHTML('beforebegin', filterBarHtml(kind));
      bar = box.previousElementSibling;
    }
    updateFilterButtons(kind);

    bar.querySelectorAll('.royal-game-filter-btn').forEach(btn => {
      if (btn.dataset.gameFilterBoundV0550 === '1') return;
      btn.dataset.gameFilterBoundV0550 = '1';
      btn.addEventListener('click', () => {
        const code = btn.dataset.gameFilter || GAME_ALL;
        if (![GAME_ALL, GAME_RM, GAME_RK].includes(code)) return;
        filters[kind] = code;
        updateFilterButtons(kind);
        try { input.blur(); } catch (_) {}
        filterRendered(kind, input.value || queries[kind] || '');
      });
    });
  }
  ...
  function activeSearchInput() {
    ...
    return (active.id === 'participantSearch' || active.id === 'teamSearch') ? active : null;
  }
  ...
  function installKeyboardDismiss() {
    if (window.__ROYAL_SEARCH_KEYBOARD_DISMISS_V0550__) return;
    window.__ROYAL_SEARCH_KEYBOARD_DISMISS_V0550__ = true;

    document.addEventListener('pointerdown', event => {
      const input = activeSearchInput();
      if (!input) return;
      if (event.target === input || event.target?.closest?.('.search-box')) return;
      dismissKeyboard();
    }, true);

    // A finger drag/scroll means the user moved focus from typing to browsing results.
    document.addEventListener('touchmove', dismissKeyboard, { passive:true, capture:true });
    document.addEventListener('scroll', dismissKeyboard, { passive:true, capture:true });
  }

  function installInput(id, kind) {
    const old = document.getElementById(id);
    if (!old) return null;
    if (old.dataset.hybridSearchV0550 === '1') {
      installFilter(kind);
      return old;
    }

    // Preserve Android-safe behavior: clone ONCE to remove app.js rerender listener,
    // then never block IME composition and never replace the field while typing.
    const input = old.cloneNode(true);
    input.value = queries[kind] || old.value || '';
    input.dataset.hybridSearchV0550 = '1';
  ...
  function installCurrent() {
    const p = installInput('participantSearch', 'participants');
    const t = installInput('teamSearch', 'teams');
    if (p) filterRendered('participants', p.value || queries.participants);
    if (t) filterRendered('teams', t.value || queries.teams);
  }

  const nativeParticipants = typeof renderParticipantsPage === 'function' ? renderParticipantsPage : null;
  if (nativeParticipants) {
    renderParticipantsPage = function renderParticipantsPageV0550(query = '') {
      const desired = String(query || queries.participants || '');
      queries.participants = desired;
      const result = nativeParticipants('');
      const input = installInput('participantSearch', 'participants');
      if (input) {
        input.value = desired;
        filterRendered('participants', desired);
      }
      return result;
    };
  }
  ...
  if (nativeTeams) {
    renderTeamsPage = function renderTeamsPageV0550(query = '') {
      const desired = String(query || queries.teams || '');
      queries.teams = desired;
      const result = nativeTeams('');
      const input = installInput('teamSearch', 'teams');
      if (input) {
        input.value = desired;
        filterRendered('teams', desired);
      }
      return result;
    };
  }
  ...
  window.RoyalHybridSearch = {
    ...
    snapshotKeys: true,
    gameFilter: true,
    getGameFilter: kind => filters[kind] || GAME_ALL,
    setGameFilter: (kind, code) => {
      if (!Object.prototype.hasOwnProperty.call(filters, kind)) return false;
      if (![GAME_ALL, GAME_RM, GAME_RK].includes(code)) return false;
      filters[kind] = code;
      updateFilterButtons(kind);
      filterRendered(kind, queries[kind] || '');
      return true;
    },
