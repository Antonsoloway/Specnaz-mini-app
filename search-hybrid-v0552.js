/* Royal CRM Mini App — Hybrid Search + Game Filter v0.5.52
 * v0.5.47-style local fallback OR snapshot searchKeys.
 * Confirmed pseudo-Russian aliases are embedded before any card cache is built.
 */
(() => {
  ...
  const ALIASES = new Map([
    ...
    ['hepbbi b hopme',['нервы в норме']],['hepbbl b hopme',['нервы в норме']],['mbl pycckue',['мы русские']],
  ...
  function ensureEmpty(container) {
    ...
    let empty = container.querySelector(':scope > .search-empty-v0552');
    if (!empty) { empty = document.createElement('div'); empty.className = 'empty-state search-empty-v0552'; empty.textContent = 'Ничего не найдено'; empty.hidden = true; container.appendChild(empty); }
    return empty;
  }
  ...
  function installFilter(kind) {
    ...
    bar.querySelectorAll('.royal-game-filter-btn').forEach(btn => {
      if (btn.dataset.gameFilterBoundV0552 === '1') return; btn.dataset.gameFilterBoundV0552 = '1';
  ...
  function installKeyboardDismiss() {
    if (window.__ROYAL_SEARCH_KEYBOARD_DISMISS_V0552__) return; window.__ROYAL_SEARCH_KEYBOARD_DISMISS_V0552__ = true;
  ...
  function installInput(id,kind) {
    ...
    if (old.dataset.hybridSearchV0552 === '1') { installFilter(kind); return old; }
    const input = old.cloneNode(true); input.value = queries[kind] || old.value || ''; input.dataset.hybridSearchV0552 = '1'; input.setAttribute('autocomplete','off'); input.setAttribute('autocapitalize','none'); input.setAttribute('spellcheck','false'); old.replaceWith(input);
  ...
  if (nativeParticipants) renderParticipantsPage = function renderParticipantsPageV0552(query='') { const desired = String(query || queries.participants || ''); queries.participants = desired; const result = nativeParticipants(''); const input = installInput('participantSearch','participants'); if (input) { input.value = desired; filterRendered('participants',desired); } return result; };
  const nativeTeams = typeof renderTeamsPage === 'function' ? renderTeamsPage : null;
  if (nativeTeams) renderTeamsPage = function renderTeamsPageV0552(query='') { const desired = String(query || queries.teams || ''); queries.teams = desired; const result = nativeTeams(''); const input = installInput('teamSearch','teams'); if (input) { input.value = desired; filterRendered('teams',desired); } return result; };
