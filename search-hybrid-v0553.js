/* Royal CRM Mini App — Hybrid Search + SearchKeys Transport v0.5.53
 * Keeps the Android-safe v0.5.52 search/filter behavior.
 * Adds one deterministic pseudo-Cyrillic reading and robust team->snapshot matching.
 */
(() => {
  ...
  let teamTimer = 0;
  let allCardCache = new WeakMap();
  let scopedCardCache = new WeakMap();
  ...
  // Exactly one extra reading for pseudo-Cyrillic names. No fuzzy search and no combinatorial variants.
  ...
  function makeLocalHaystack(text) {
    ...
    if (/[a-z]/.test(base)) {
      const read = englishRead(base); if (read) { parts.add(read); parts.add(read.replace(/\s+/g,'')); }
      const pseudo = pseudoRead(base); if (pseudo) { parts.add(pseudo); parts.add(pseudo.replace(/\s+/g,'')); }
    }
  ...
  function resetCaches() {
    allCardCache = new WeakMap();
    scopedCardCache = new WeakMap();
  }
  ...
  function refreshMaps() {
    ...
    if (participantSource !== participants) {
      participantSource = participants; participantById = new Map(); changed = true;
    ...
    if (teamSource !== teams) {
      teamSource = teams; teamByName = new Map(); changed = true;
    ...
    if (changed) resetCaches();
  }
  function teamForCard(card) {
    refreshMaps();
    const names = [];
    let dataName = String(card?.dataset?.team || '');
    try { dataName = decodeURIComponent(dataName); } catch (_) {}
    if (dataName) names.push(dataName);
    const visibleName = card?.querySelector?.('.team-card-main strong')?.textContent || card?.querySelector?.('strong')?.textContent || '';
    if (visibleName) names.push(visibleName);

    let candidates = [];
    for (const name of names) {
      const list = teamByName.get(normalize(name)) || [];
  ...
  function ensureEmpty(container) {
    ...
    let empty = container.querySelector(':scope > .search-empty-v0553');
    if (!empty) { empty = document.createElement('div'); empty.className = 'empty-state search-empty-v0553'; empty.textContent = 'Ничего не найдено'; empty.hidden = true; container.appendChild(empty); }
    return empty;
  }
  ...
  function installFilter(kind) {
    ...
    bar.querySelectorAll('.royal-game-filter-btn').forEach(btn => {
      if (btn.dataset.gameFilterBoundV0553 === '1') return; btn.dataset.gameFilterBoundV0553 = '1';
  ...
  function installKeyboardDismiss() {
    if (window.__ROYAL_SEARCH_KEYBOARD_DISMISS_V0553__) return; window.__ROYAL_SEARCH_KEYBOARD_DISMISS_V0553__ = true;
  ...
  function installInput(id,kind) {
    ...
    if (old.dataset.hybridSearchV0553 === '1') { installFilter(kind); return old; }
    const input = old.cloneNode(true); input.value = queries[kind] || old.value || ''; input.dataset.hybridSearchV0553 = '1'; input.setAttribute('autocomplete','off'); input.setAttribute('autocapitalize','none'); input.setAttribute('spellcheck','false'); old.replaceWith(input);
  ...
  if (nativeParticipants) renderParticipantsPage = function renderParticipantsPageV0553(query='') { const desired = String(query || queries.participants || ''); queries.participants = desired; const result = nativeParticipants(''); const input = installInput('participantSearch','participants'); if (input) { input.value = desired; filterRendered('participants',desired); } return result; };
  const nativeTeams = typeof renderTeamsPage === 'function' ? renderTeamsPage : null;
  if (nativeTeams) renderTeamsPage = function renderTeamsPageV0553(query='') { const desired = String(query || queries.teams || ''); queries.teams = desired; const result = nativeTeams(''); const input = installInput('teamSearch','teams'); if (input) { input.value = desired; filterRendered('teams',desired); } return result; };
  ...
  window.RoyalHybridSearch = {
    ...
    localFallback:true,
    snapshotKeys:true,
    deterministicPseudoRead:true,
    gameFilter:true,
    getGameFilter:kind=>filters[kind]||GAME_ALL,
    setGameFilter:(kind,code)=>{if(!Object.prototype.hasOwnProperty.call(filters,kind)||![GAME_ALL,GAME_RM,GAME_RK].includes(code))return false;filters[kind]=code;updateFilterButtons(kind);filterRendered(kind,queries[kind]||'');return true;},
