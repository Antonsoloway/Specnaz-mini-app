/* Royal CRM Mini App — Specnaz v0.5.17, Telegram ID only */
(() => {
  ...
  function renderHeroes() {
    ...
    const cards = list.map((p, index) => {
      ...
      return `<article class="hero-card"><div class="hero-place${place <= 3 ? ' medal' : ''}">${medal(place)}</div>${avatarHtml(p, 'hero-avatar')}<div class="hero-main"><strong>${esc(displayName(p))}</strong>${usernameButtonHtml(p)}<div class="hero-rank">${esc(rank)}</div></div><div class="hero-score"><b>${trips}</b><small>ПОХОДОВ</small></div></article>`;
    }).join('');
  ...
  function historyRowHtml(row) {
    ...
    return `<article class="history-row"><div class="history-row-head">${avatar}<div class="history-person"><strong>${esc(name)}</strong><div class="history-date">${esc(row?.date || '')}</div></div><span class="history-rank">${esc(row?.rank || '')}</span></div>${row?.team ? `<div class="history-team">${esc(row.team)}</div>` : ''}<div class="history-scoreline"><span>Было <b>${esc(before)}</b></span><span>→</span><span>Стало <b>${esc(after)}</b></span>${added ? `<span class="history-added">+${esc(added)}</span>` : ''}</div>${row?.message ? `<div class="history-message">${historyMessageHtml(row)}</div>` : ''}</article>`;
  }
