/* Royal CRM Mini App v0.6.1 — team-list photo previews, symbol-safe identity v2 */
(() => {
  'use strict';
  if (String(window.__ROYAL_BUILD__ || '') !== '0.6.1') return;
  if (window.__ROYAL_TEAM_LIST_PREVIEW_V061_V2__) return;

  const VERSION = '0.6.1-team-list-preview.2-symbol-safe';
  const CONCURRENCY = 3;
  ...
  function keyFor(team, name, teamGameValue, token) {
    const sourceKey = clean(team?.key);
    const identity = sourceKey
      ? `key:${identityText(sourceKey)}`
      : `name:${identityText(name)}\ngame:${identityText(game(teamGameValue))}`;
    return `v2|${identity}|${clean(token)}`;
  }
  ...
  function findTeam(ref) {
    const wantedIdentity = identityText(ref?.name);
    const wantedSearch = searchText(ref?.name);
    const wantedGame = game(ref?.game);
    if (!wantedIdentity) return null;

    const teams = publicTeams();
    const byGame = wantedGame
      ? teams.filter(team => teamGame(team) === wantedGame)
      : teams;

    const exact = byGame.find(team => identityText(team?.name) === wantedIdentity);
    if (exact) return exact;

    // Compatibility for harmless spacing/case differences only. If multiple
    // teams collapse to the same search text (for example ⚓️ and 🎓 variants),
    // refuse the fallback instead of borrowing another team's photo.
    const fuzzy = byGame.filter(team => searchText(team?.name) === wantedSearch);
  ...
  function identityForCard(card) {
    ...
    return {
      ...
      photoUrl,
      token,
      key:keyFor(team, name, teamGameValue, token)
    };
  }
  ...
  function installCss() {
    if (document.querySelector('style[data-team-list-preview-v061-v2="1"]')) return;
    const style = document.createElement('style');
    style.dataset.teamListPreviewV061V2 = '1';
    style.textContent = `
  ...
  try {
    if (typeof renderTeamsPage === 'function') {
      ...
      renderTeamsPage = function renderTeamsPageWithSymbolSafePreview(...args) {
  ...
  window.__ROYAL_TEAM_LIST_PREVIEW_V061_V2__ = VERSION;
})();
