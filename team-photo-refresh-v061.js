/* Royal CRM Mini App — v0.6.1 team photo replacement/cache bridge
 * Team-photo identity is name+game, but photo content is versioned separately.
 * A replaced photo must become visible immediately instead of waiting for the
 * legacy 30-minute persistent-media refresh window.
 */
(() => {
  'use strict';
  if (String(window.__ROYAL_BUILD__ || '') !== '0.6.1') return;

  const VERSION = '0.6.1-team-photo-refresh.1';
  ...
  const baseKey = (name, game) => `${normText(name)}\n${normText(normGame(game))}`;
  ...
  function recordFor(mode, name, game) {
    const wantedName = normText(name);
    const wantedGame = normGame(game);
    const list = mode === 'admin' ? adminTeams() : publicTeams();
    return list.find(item => normText(item?.name) === wantedName && normGame(item?.game || item?.games?.[0]) === wantedGame)
      || list.find(item => normText(item?.name) === wantedName)
      || null;
  }
  ...
  function apply(img, cacheKey, entry, mode) {
    ...
    if (mode === 'admin') {
      ...
      img.dataset.adminMediaCache = 'v061-versioned';
    } else {
      img.dataset.teamProxyLoaded = '1';
      img.dataset.mediaCache = 'v061-versioned';
    }
  ...
  function publicLoader() {
    ...
    return loadVersioned(img, 'public').catch(error => {
      console.warn('v0.6.1 public team photo refresh failed:', error?.message || error);
      return false;
    });
  }
  ...
  function installFetchObserver() {
    ...
    window.fetch = async function v061TeamPhotoWriteObserver(input, init) {
