...
async function handleAdminTeamPhoto(request, env, ctx, url) {
  ...
  const team = teams.find(item =>
    normalizeTeam(item?.name) === wantedName &&
  ...
  if (repo && token && /^[0-9a-f]{64}$/.test(hash)) {
    try {
      ...
      if (github.ok) {
        ...
        const type = detectImageType(bytes,github.headers.get('Content-Type'));
        if (bytes.length && type) return imageResponse(adminResponse,bytes,type,'SHA256-MEDIA');
      }
    } catch (error) {
      console.warn('admin team photo media read failed',error?.message || 'unknown');
    }
  }

  // Compatibility for old team rows whose private hash file has not yet been
  // materialized. The URL comes from the private admin snapshot / Команды!C.
  try {
    ...
    if (!upstream.ok) return imageError(adminResponse,'TEAM_PHOTO_SOURCE_FAILED',upstream.status === 404 ? 404 : 502);
    const bytes = new Uint8Array(await upstream.arrayBuffer());
    const type = detectImageType(bytes,upstream.headers.get('Content-Type'));
    if (!bytes.length || bytes.length > 8*1024*1024 || !type) {
      return imageError(adminResponse,'TEAM_PHOTO_SOURCE_INVALID',415);
    }
    return imageResponse(adminResponse,bytes,type,'SOURCE-FALLBACK');
  } catch (error) {
