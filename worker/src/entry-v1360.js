  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/health' && request.method === 'GET') {
      ...
      return jsonFrom(base, {
        ...
        contactById: 'direct-session-verified-private-snapshot+privacy-aware-bot-profile-button'
      }, 200);
    }
    return base;
  }
...
async function handleContactById(request, env) {
  ...
  try {
    ...
    if (targetUsername) {
      return json({ ok:false, error:'USERNAME_AVAILABLE', message:'У участника уже есть @username — используйте обычную ссылку.' }, 409, cors);
    }
    ...
    if (previous && now - previous < 4000) {
      return json({ ok:true, sent:false, duplicate:true, botUsername:BOT_USERNAME, message:'Ссылка уже отправлена Голубцом.' }, 200, cors);
    }
    ...
    const response = await fetch(`${botApi}/sendMessage`, {
      ...
      body: JSON.stringify({
        ...
        reply_markup: { inline_keyboard: [[{ text:'👤 Открыть профиль', url:`tg://user?id=${targetId}` }]] }
      }),
      cache: 'no-store'
    });
    ...
    if (!response.ok || !telegram?.ok) {
      ...
      const privacyRestricted = /BUTTON_USER_PRIVACY_RESTRICTED|privacy setting.*user|user.*privacy/i.test(description);
      if (privacyRestricted) {
        console.warn('contact-by-id profile button blocked by target privacy settings');
        try {
          await fetch(`${botApi}/sendMessage`, {
            ...
            body: JSON.stringify({
              chat_id: requesterId,
              text: `Связаться с участником: ${targetName}\n\nTelegram не разрешает создать кнопку открытия профиля по ID из-за настроек приватности этого участника. Это ограничение Telegram, приложение не может его обойти.`,
              disable_notification: true
            }),
            cache: 'no-store'
          });
        ...
        return json({
          ok: false,
          error: 'TARGET_PROFILE_PRIVACY_RESTRICTED',
          privacyRestricted: true,
          message: 'У этого участника Telegram запрещает открыть профиль по ID из-за его настроек приватности. Это ограничение Telegram, а не ошибка приложения.'
        }, 409, cors);
      }
...
async function hmacSha256(secret, value) {
  ...
  if (!hmacKeyPromise || hmacKeySecret !== normalized) {
    ...
    hmacKeyPromise = crypto.subtle.importKey('raw', new TextEncoder().encode(normalized), {name:'HMAC',hash:'SHA-256'}, false, ['sign']);
  }
...
function corsPreflight(request, env) { return new Response(null, {status:204, headers:corsHeaders(request,env)}); }
function requireSecret(value, name) { if (!String(value || '').trim()) throw appError(500, `${name}_MISSING`); }
function appError(status, code, publicMessage='') { const error=new Error(code); error.status=status; error.code=code; error.publicMessage=publicMessage; return error; }
function json(data, status=200, headers={}) { const out=new Headers(headers); out.set('Content-Type','application/json; charset=utf-8'); out.set('X-Content-Type-Options','nosniff'); out.set('Cache-Control','no-store'); return new Response(JSON.stringify(data), {status, headers:out}); }
function jsonFrom(response,payload,status){ const headers=new Headers(response.headers); headers.set('Content-Type','application/json; charset=utf-8'); headers.set('Cache-Control','no-store'); headers.delete('Content-Length'); headers.delete('ETag'); return new Response(JSON.stringify(payload),{status,headers}); }
...
function base64UrlDecode(value){ let text=String(value||'').replace(/-/g,'+').replace(/_/g,'/'); while(text.length%4)text+='='; const binary=atob(text); return Uint8Array.from(binary,char=>char.charCodeAt(0)); }
